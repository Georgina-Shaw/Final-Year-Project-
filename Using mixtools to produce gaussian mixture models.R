...
#a univariate symmetric, loaction-shifted semiparametric example
...
#how bandwidth effect the plots 
...
#illustration of cutoff points 
data("Waterdata")
cutpts <- 10.5*(-6:6)
#make multinormal data set from original
watermult <- makemultdata(Waterdata, cuts = cutpts)

#find the multinormal parameters via EM algorithm 
set.seed(15)
...
#calc and plots the estimates distribution function 
...
#a triviate gaussian example 
#nonparameric example
...
#more general multivariate nonparametric example 
m <- 2; r <- 5
lambda <- c(0.4, 0.6)
df <- c(2, 10); ncp <- c(0, 8)
sh1 <- c(1, 1) ; sh2 <- c(1, 5)

#generate a pseudo-random sample of size n = 300
...
#plotting resulting density estimates 
plot(b, breaks = 15)

#compute the ISE of the estimated density 
par(mfrow=c(2,2))
for (j in 1:2){
...
#an example of fitting 2-component model and a bootstrapping approach 
data("CO2data")
attach(CO2data)
...
#implementation of the IGLE algorithm 
